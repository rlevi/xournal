    ...
    if (pg->bg->type == BG_SOLID) {
...
#ifdef USE_HILDON
            gchar descr[strlen(_("Could not write background ''. Continuing anyway."))+
		    strlen(tmpfn)];
            sprintf (descr, _("Could not write background '%s'. Continuing anyway."), tmpfn);
      ...
      if (!is_clone) {
        if (pg->bg->file_domain == DOMAIN_ATTACH) {
          ...
          if (!success) {
#ifdef USE_HILDON
            gchar descr[strlen("Could not write background ''. Continuing anyway.")+
		    strlen (tmpfn)];
            sprintf (descr, "Could not write background '%s'. Continuing anyway.", tmpfn);
            dialog = hildon_note_new_information (GTK_WINDOW (winMain), descr);
#else
            dialog = gtk_message_dialog_new(GTK_WINDOW(winMain), GTK_DIALOG_MODAL,
              GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, 
...
#ifdef USE_HILDON
              gchar descr[strlen(_("Could not open background ''. Setting background to white."))+
		      strlen(tmpbg_filename)];
              sprintf (descr, _("Could not open background '%s'. Setting background to white."),
		      tmpbg_filename);
              dialog = hildon_note_new_information (GTK_WINDOW (winMain), descr);
#else
              dialog = gtk_message_dialog_new(GTK_WINDOW(winMain), GTK_DIALOG_MODAL,
...
gboolean user_wants_second_chance(char **filename)
{
  ...
  gchar descr[strlen(_("Could not open background ''.\nSelect another file?"))+strlen(*filename)];
  sprintf (descr, _("Could not open background '%s'.\nSelect another file?"), *filename);
  dialog = hildon_note_new_information (GTK_WINDOW (winMain), descr);
  ...
  model = HILDON_FILE_SYSTEM_MODEL(g_object_new(HILDON_TYPE_FILE_SYSTEM_MODEL,
                   "ref_widget", GTK_WINDOW(winMain), NULL));
...
#ifdef USE_HILDON
  banner=hildon_banner_show_information (GTK_WIDGET(winMain), NULL, _("Opening file..."));
...
#ifdef USE_HILDON
      gchar descr[strlen(_("Could not open background ''."))+strlen(tmpfn)];
      sprintf (descr, _("Could not open background '%s'."), tmpfn);
      dialog = hildon_note_new_information (GTK_WINDOW (winMain), descr);
#else
      dialog = gtk_message_dialog_new(GTK_WINDOW(winMain), GTK_DIALOG_MODAL,
...
//    printf("DEBUG: Processing request for page %d at %f dpi\n", req->pageno, req->dpi);
  ...
  if (pixbuf != NULL) { // success
    ...
    if (!bgpdf.has_failed) {
#ifdef USE_HILDON
      gchar descr[strlen(_("Unable to render one or more PDF pages."))];
      sprintf (descr, _("Unable to render one or more PDF pages."));
      dialog = hildon_note_new_information (GTK_WINDOW (winMain), descr);
#else
      dialog = gtk_message_dialog_new(GTK_WINDOW(winMain), GTK_DIALOG_MODAL,
...
void save_config_to_file(void)
{
  ...
  update_keyval("paper", "progressive_bg",
    _(" progressive scaling of bitmap backgrounds (true/false)"),
