...
#ifdef USE_HILDON
void recolor_selection(GdkColor *color_no, guint color_rgba)
#else
...
gboolean do_hand_scrollto(gpointer data)
{
  ...
  hildon_pannable_area_scroll_to (canvas, ui.hand_scrollto_cx, ui.hand_scrollto_cy);
  g_object_set (scrolledwindowMain, "mov-mode", !HILDON_MOVEMENT_MODE_VERT | !HILDON_MOVEMENT_MODE_HORIZ);
#else
*/
...
void start_text(GdkEvent *event, struct Item *item)
{
  ...
  update_font_button();
#ifdef USE_HILDON
  gtk_widget_set_sensitive(GET_COMPONENT("Paste"), FALSE);
#else
...
void end_text(void)
{
  ...
  ui.cur_item_type = ITEM_NONE;
#ifdef USE_HILDON
  gtk_widget_set_sensitive(GET_COMPONENT("Paste"), TRUE);
#else
