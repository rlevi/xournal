...
//#ifdef USE_HILDON
  // On the Nokia Internet Tablets
  // XInput only reports absolute coordinates, so we need to add scrolling
  // offset
//  *px += sx;
//  *py += sy;
//#endif
...
#ifndef USE_HILDON
    /* For some reason Maemo 5 GTK 2.14 does not have this issue */
...
#ifdef USE_HILDON
// Maemo utility functions
guint gdk_color_to_rgb (GdkColor *color)
{
  return GNOME_CANVAS_COLOR (color->red / 0x101, color->green / 0x101, color->blue / 0x101);
}

guint gdk_color_to_rgba (GdkColor *color, guint alpha_mask)
{
  return GNOME_CANVAS_COLOR_A (color->red / 0x101, color->green / 0x101, color->blue / 0x101, alpha_mask);
}
 ...
 * gchar *gdk_color_to_string (GdkColor *color)
{
  ...
  color_str = malloc (sizeof ("#xxxxxx"));

  sprintf (color_str, "#%02s%02s%02s", color->red, color->green, color->blue);
...
void update_color_buttons(void)
{
  ...
  rgb_to_gdkcolor (ui.cur_brush->color_rgba, &color);

  /* update color chooser with currently selected brush color */
//  hildon_color_button_set_color (HILDON_COLOR_BUTTON(GET_COMPONENT("Color")), &color);
#else
...
void update_tool_buttons(void)
{
  ...
  gtk_toggle_tool_button_set_active(
      GTK_TOGGLE_TOOL_BUTTON(GET_COMPONENT("buttonPressure")), 
...
void update_page_stuff(void)
{
  ...
  while (ui.layerbox_length > ui.cur_page->nlayers+1) {
    /* Why the heck the do not provide hildon_touch_selector_remove_text !! */
    GtkTreeModel *model;
    GtkTreePath *path;
    GtkTreeIter iter;
    GtkRequisition requisition;

    model = hildon_touch_selector_get_model (HILDON_TOUCH_SELECTOR(layerbox), 0);
...
//   gtk_widget_set_size_request (GTK_WIDGET(layerbox), -1, -1);
  ...
  while (ui.layerbox_length < ui.cur_page->nlayers+1) {
    g_snprintf(tmp, 10, _("Layer %d"), ui.layerbox_length++);
    hildon_touch_selector_prepend_text (HILDON_TOUCH_SELECTOR(layerbox), tmp);
  }

  hildon_touch_selector_set_active(HILDON_TOUCH_SELECTOR(layerbox), 0,
		  ui.cur_page->nlayers-1-ui.layerno);
#endif
...
#ifdef USE_HILDON
  gtk_widget_set_sensitive(GET_COMPONENT("FirstPage"), ui.pageno!=0);
  gtk_widget_set_sensitive(GET_COMPONENT("PreviousPage"), ui.pageno!=0);
  gtk_widget_set_sensitive(GET_COMPONENT("NextPage"), TRUE);
  gtk_widget_set_sensitive(GET_COMPONENT("LastPage"), ui.pageno!=journal.npages-1);
  gtk_widget_set_sensitive(GET_COMPONENT("Paste"), ui.cur_layer!=NULL);
#else
...
#ifdef USE_HILDON
  gtk_toggle_tool_button_set_active(
    GTK_TOGGLE_TOOL_BUTTON(GET_COMPONENT("Fullscreen")), ui.fullscreen);
#else
  gtk_toggle_tool_button_set_active(
...
{
#ifdef USE_HILDON
  gtk_widget_set_sensitive(GET_COMPONENT("Cut"), ui.selection!=NULL);
  gtk_widget_set_sensitive(GET_COMPONENT("Copy"), ui.selection!=NULL);
  gtk_widget_set_sensitive(GET_COMPONENT("Paste"), ui.cur_item_type!=ITEM_TEXT);
  gtk_widget_set_sensitive(GET_COMPONENT("Delete"), ui.selection!=NULL);
#else
...
#ifdef USE_HILDON
// Maemo uses GdkColors
void hildon_set_cur_color (GdkColor *color)
{
  ui.cur_brush->color_no = COLOR_OTHER;
  ui.cur_brush->color_rgba = gdk_color_to_rgba (color, 0xFF);
...
void hildon_process_color_activate(GtkWidget *menuitem, GdkColor *color)
{
  ...
  if (ui.cur_item_type == ITEM_TEXT)
    recolor_temp_text (COLOR_OTHER, gdk_color_to_rgba (color, 0xFF));

  if (ui.selection != NULL) {
    recolor_selection(color, gdk_color_to_rgba (color, 0xFF));
    update_color_buttons();
    update_color_menu();
  }
...
gboolean ok_to_close(void)
{
  ...
  filename = (ui.filename != NULL) ? ui.filename : _("Untitled");

  descr = (char *) malloc (strlen(_("Save changes to ''?"))+ strlen (filename));
  sprintf (descr, _("Save changes to '%s'?"), filename);

  dialog = hildon_note_new_confirmation (GTK_WINDOW (winMain), descr);
  hildon_note_set_button_texts (HILDON_NOTE(dialog), _("Yes"), _("No"));
